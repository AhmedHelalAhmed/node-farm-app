...
const server = http.createServer((request, response) => {
    ...
    if (pathName === '/' || pathName === '/overview') {
        const cardsHTML = data.map((product) => replaceTemplate(views.cardTemplate, product)).join('');
        const output = views.overviewTemplate.replace(/{%PRODUCT_CARDS%}/g, cardsHTML)
        response.end(output);
...
server.listen(config.PORT_NUMBER, config.HOSTNAME, () => {
    console.log(`Listening on ${config.HOSTNAME} to requests on port ${config.PORT_NUMBER}`);
});